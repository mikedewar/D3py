...
class D3object(object):
    ...
    def save(self, where=None):
        if where is not None and not os.path.isdir(where):
            try:
                ...
                print "Could not create directory structure %s: %s" % (
                    where, e)
    ...
    def __exit__(self, ex_type, ex_value, ex_tb):
        if ex_tb is not None:
            print "Cleanup after exception: %s: %s" % (
                ex_type, ex_value)
        self.cleanup()
...
class Figure(D3object):
    ...
    def __init__(self, data, name="figure",
                 width=400, height=100, port=8000,
                 template=None, font="Asap", **kwargs):
        """
        data : dataFrame
        ...
        self.draw += "var margin = %s;" % \
                json.dumps(self.margins).replace('""', '')
    ...
    def build_scales(self):
        ...
        for colname in self.data.columns:
            ...
            if type(self.data[colname][0]) is str:
                ...
                height_linspace = np.linspace(
                    height, 0, len(self.data[colname]))
                ...
                scale.update({
                    "%s_y" % colname: str(y_range),
                    "%s_x" % colname: str(x_range),
                })
            else:
                y_range = JS.Object("d3.scale") \
                ...
                if min(self.data[colname]) < 0:
                    x_range.add_attribute(
                        "domain", [
                            min(self.data[colname]),
                            max(self.data[colname])
                        ])
                    y_range.add_attribute(
                        "domain", [
                            max(self.data[colname]),
                            min(self.data[colname])
                        ])
                    ...
                    x_range.add_attribute(
                        "domain", [0, max(self.data[colname])])
                    y_range.add_attribute(
                        "domain", [max(self.data[colname]), 0])
    ...
    def build_js(self):
        ...
        draw_code += "var scales = %s;" % json.dumps(
            scale, sort_keys=True, indent=4).replace('"', '')
    ...
    def data_to_json(self):
        ...
        try:
            ...
            print "Error: Overflow on variable (type %s): %s: %s" % (
                type(d), d, e)
            raise
        return s
    ...
    def save_data(self, directory=None):
        ...
        # write data
        fname = "%s/%s.json" % (
            directory or self.work_dir, self.name)
        fh = open(fname, 'w+')
    ...
    def show(self, interactive=None):
        ...
        if blocking:
            ...
            # fire up a browser
            webbrowser.open_new_tab("http://localhost:%s/%s.html" % (
                self.port, self.name))
    ...
    def serve(self, blocking=True):
        ...
        """
        if self.server_thread is None or \
           self.server_thread.active_count() == 0:
            ...
            if blocking:
                ...
                print "You can find your chart at " + \
                      "http://localhost:%s/%s/%s.html" % (
